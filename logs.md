...
1. _Base class for IO Multiplexing_
2. 给所有 IO 复用保留统一的接口，_Must be called in the loop thread._
3. `poll()` 方法负责 **_Polls the I/O events._**
4. `updateChannel()` 方法负责 **_Changes the interested I/O events._**
5. `removeChannel()` 方法负责 **_Remove the channel, when it destructs._**
...
**_EpollPoller is IO Multiplexing with epoll(4)._**
...
由于 _one loop per thread_ 的设计，所以需要一个类来**获取当前线程的线程 ID**
...
_EventLoop : Reactor, at most one per thread. This is an interface class, so don't expose too much details._
...
4. [`unique_ptr`](https://github.com/Corner430/study-notes/blob/main/cpp/cpp高级笔记.md#233-unique_ptr) 用来保证只有一个 Poller 对象和 wakeupChannel\_ 对象
...
13. [`snprintf`, `sprintf`](https://zh.cppreference.com/w/c/io/fprintf)
...
_Acceptor of incoming TCP connections._
...
_All client visible callbacks go here._
